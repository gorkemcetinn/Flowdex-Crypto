...
def market_overview(limit: int = Query(8, ge=1, le=20)) -> list[dict]:
    """Return the top assets ranked by market cap."""

    return market_data.get_overview(limit)


@router.get("/top-movers", response_model=TopMoversResponse)
def top_movers(limit: int = Query(4, ge=1, le=6)) -> TopMoversResponse:
    """Return the leading gainers and laggards for the session."""

    payload = market_data.get_top_movers(limit)
...
def watchlist_with_quotes(user_id: str, db: Session = Depends(get_db)) -> list[dict]:
    ...
    if not symbols:
        return []
    return market_data.get_watchlist_snapshots(symbols)
...
async def stream_watchlist(
    ...
    for symbol in requested:
        if symbol not in unique_symbols and market_data.available_symbol(symbol):
    ...
    async def event_generator() -> AsyncIterator[str]:
        async for event in market_data.stream_quotes(unique_symbols, iterations=max_events, delay=delay):
...
def market_detail(symbol: str) -> dict:
    ...
    asset = market_data.get_asset_detail(symbol)
    if not asset:
